...
class ThreadService {
  ...
  static async getFallbackThreads(agentId: string): Promise<ThreadData[]> {
  ...
  static async searchThreads(agentId: string, query: string, options?: { type: 'fallback' | 'conversation' }): Promise<ThreadData[]> {
  ...
  static async updateThreadName(threadId: string, name: string): Promise<ThreadData | undefined> {
  ...
  static async getThreadById(threadId: string): Promise<ThreadData | undefined> {
    if (!threadId) throw new Error("Thread ID is required");
  ...
  static async getUserThreads(userId: string, assistantId?: string): Promise<ThreadData[]> {
  ...
  static async createThread(data: ThreadData): Promise<ThreadData> {
